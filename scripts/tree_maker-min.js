...
function treeMaker(tree, params) {
  ...
  const trad =
    treeParams[Object.keys(tree)[0]] !== undefined &&
    treeParams[Object.keys(tree)[0]].trad !== undefined
      ? treeParams[Object.keys(tree)[0]].trad
      : Object.keys(tree)[0].trad;
  card.innerHTML = `<p class="tree__container__step__card__p" onClick="display(${
    Object.keys(tree)[0]
  })" id="card_${Object.keys(tree)[0]}">${trad}</p>`;
...
function iterate(tree, start, from) {
  ...
  for (const key in tree) {
    const textCard =
      treeParams[key] !== undefined && treeParams[key].trad !== undefined
        ? treeParams[key].trad
        : key;

    if (!document.getElementById(`card_${key}`)) {
      treeContainer.innerHTML += `<div class="tree__container__step"><div class="tree__container__step__card" onClick="display(${key})"  id="${key}"><p id="card_${key}"  class="tree__container__step__card__p">${textCard}</p></div></div>`;
    ...
    if ((from && !start) || start) {
      ...
      allLinks.push([
        "path" + pathNumber,
        from ? from : "tree__container__step__card__first",
        key,
      ]);
      pathNumber++;
    }
